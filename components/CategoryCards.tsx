    ...
    <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:14 }}>
      {CATS.map(c => (
        <Link key={c.name} href={c.href} className="cat-card">
          ...
          <div style={{ fontSize:16, fontWeight:700, color:"#0f172a", marginBottom:6 }}>{c.name}</div>
          <div style={{ fontSize:12, color:"#64748b", marginBottom:14, lineHeight:1.5 }}>{c.desc}</div>
