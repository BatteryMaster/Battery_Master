    ...
    <main style={{ minHeight:"100vh", background:"#f0fdf4" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#052e16,#14532d)", padding:"48px 0 44px" }}>
        <div className="wrap">
          ...
          <h1 style={{ fontSize:"clamp(24px,4vw,40px)", fontWeight:900, color:"#fff", marginBottom:10 }}>About Battery Master</h1>
          <p style={{ fontSize:15, color:"rgba(255,255,255,0.6)" }}>Karachi's trusted battery & EV parts store</p>
        </div>
      </div>

      <div className="wrap" style={{ paddingTop:36, paddingBottom:80, maxWidth:820 }}>

        <div style={{ background:"#fff", border:"1.5px solid #d1fae5", borderRadius:16, padding:"36px 32px", marginBottom:20 }}>
          <h2 style={{ fontSize:19, fontWeight:800, color:"#0f172a", marginBottom:16 }}>🔋 Who We Are</h2>
          ...
          <p style={{ fontSize:15, color:"#475569", lineHeight:1.9, marginBottom:16 }}>
            We serve customers who need the best quality batteries and EV parts for their vehicles, solar systems, or electronics — at the right price. We always stock Grade-A LiFePO4 cells (EVE, CATL), JK BMS, Samsung/LG 18650 cells, and e-bike conversion kits.
          </p>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))", gap:14, marginBottom:20 }}>
          {[
            { icon:"🛡️", title:"JK BMS Specialist",   desc:"4S to 24S, with active balancing." },
            ...
            { icon:"🔧", title:"Wires & Tools",          desc:"Silicon wires, terminals, testers." },
            { icon:"💬", title:"WhatsApp Support",        desc:"Direct contact for orders and advice." },
          ].map(f => (
            <div key={f.title} style={{ background:"#fff", border:"1.5px solid #d1fae5", borderRadius:13, padding:"20px 16px" }}>
        ...
        <div style={{ background:"#fff", border:"1.5px solid #d1fae5", borderRadius:16, padding:"28px 26px", marginBottom:20 }}>
          <h2 style={{ fontSize:16, fontWeight:800, color:"#0f172a", marginBottom:18 }}>📋 Contact Details</h2>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))", gap:18 }}>
            {[
              ...
              { icon:"🕐", label:"Hours",    val:"Mon–Sat 10am–8pm | Sunday: Closed" },
            ].map(x => (
              <div key={x.label}>
                <div style={{ fontSize:20, marginBottom:5 }}>{x.icon}</div>
                <div style={{ fontSize:10, fontWeight:700, letterSpacing:".08em", textTransform:"uppercase", color:"#94a3b8", marginBottom:3 }}>{x.label}</div>
                <div style={{ fontSize:12.5, fontWeight:600, color:"#0f172a", lineHeight:1.5 }}>{x.val}</div>
              </div>
            ))}
          </div>
        </div>
