...
const SLUG_TO_CAT: Record<string, { name: string; icon: string; desc: string }> = {
  ...
  "lifepo4-cell":           { name:"LiFePO4 Cell",           icon:"🌱", desc:"EVE LF105, LF280K, CATL 200Ah, 304Ah Grade-A — best for solar & EV" },
  ...
  return (
    <main style={{ minHeight:"100vh", background:"#f0fdf4" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#052e16,#14532d)", padding:"36px 0 32px" }}>
        <div className="wrap">
          <nav style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:"rgba(255,255,255,0.5)", marginBottom:14, flexWrap:"wrap" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:14, flexWrap:"wrap" }}>
            <div style={{ width:52, height:52, background:"rgba(255,255,255,0.12)", borderRadius:14, display:"flex", alignItems:"center", justifyContent:"center", fontSize:26, flexShrink:0 }}>
              {cat.icon}
            </div>
            <div>
              <h1 style={{ fontSize:"clamp(22px,4vw,34px)", fontWeight:900, color:"#fff", marginBottom:5 }}>{cat.name}</h1>
              <p style={{ fontSize:13, color:"rgba(255,255,255,0.6)" }}>{cat.desc}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="wrap" style={{ paddingTop:28, paddingBottom:80 }}>
        <div style={{ fontSize:13, color:"#64748b", marginBottom:18, fontWeight:500 }}>
          <span style={{ fontWeight:800, color:"#0f172a" }}>{catProducts.length}</span> products in {cat.name}
        </div>
        ...
        {catProducts.length > 0 ? (
          ...
          <div style={{ textAlign:"center", padding:"60px 20px", color:"#94a3b8" }}>
            ...
            <div style={{ fontSize:13, marginTop:6, marginBottom:18 }}>Send a WhatsApp inquiry</div>
            <a href="https://wa.me/923329891510" target="_blank" rel="noopener noreferrer"
              style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"11px 22px", background:"#25D366", color:"#fff", borderRadius:8, fontSize:14, fontWeight:700, textDecoration:"none" }}>
              📱 WhatsApp Inquiry
            </a>
          </div>
        )}
        ...
        <div style={{ marginTop:28, display:"flex", gap:10, flexWrap:"wrap" }}>
          <Link href="/categories" style={{ display:"inline-flex", alignItems:"center", padding:"10px 18px", background:"#fff", color:"#0f172a", border:"1.5px solid #d1fae5", borderRadius:8, fontSize:13, fontWeight:700, textDecoration:"none" }}>
            ← All Categories
          </Link>
          <a href="https://wa.me/923329891510" target="_blank" rel="noopener noreferrer"
            style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"10px 18px", background:"#25D366", color:"#fff", borderRadius:8, fontSize:13, fontWeight:700, textDecoration:"none" }}>
            📱 WhatsApp Order
          </a>
        </div>
      </div>
