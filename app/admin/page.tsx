...
export default function AdminPage() {
  ...
  // Seed from products.ts if table is empty
        ...
        <div style={{ padding:20, display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div>
            ...
            <div onClick={() => fRef.current?.click()}
              style={{ border:"2px dashed #d1fae5", borderRadius:12, minHeight:110, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:8, cursor:"pointer", background:"#f0fdf4", padding:14 }}>
              {p.image ? <img src={p.image} alt="preview" style={{ maxHeight:120, maxWidth:"100%", objectFit:"contain", borderRadius:8 }} />
                : <><div style={{ fontSize:30 }}>📷</div><div style={{ fontSize:13, fontWeight:600, color:"#64748b" }}>Tap to upload</div></>}
            </div>
            <input ref={fRef} type="file" accept="image/*" style={{ display:"none" }} onChange={e => handleImg(e, p, setP)} />
            <input value={p.image.startsWith("data:") ? "" : p.image} onChange={e => setP({ ...p, image:e.target.value })}
              placeholder="Or paste image URL: /products/item.jpg"
            ...
            {p.image && <button onClick={() => setP({ ...p, image:"" })}
          ...
          <div>
            ...
            <input value={p.name} onChange={e => setP({ ...p, name:e.target.value })} placeholder="e.g. JK BMS 4S 100A"
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            <div>
              ...
              <input value={p.price.replace(/^Rs\.\s*/,"")} inputMode="numeric" placeholder="e.g. 4500"
                onChange={e => setP({ ...p, price:fmtPrice(e.target.value) })}
                style={{ width:"100%", padding:"11px 13px", border:"1.5px solid #e2e8f0", borderRadius:9, fontSize:14, outline:"none", fontFamily:"inherit", boxSizing:"border-box" }} />
              {p.price && <div style={{ fontSize:11, color:"#16a34a", marginTop:4, fontWeight:600 }}>{p.price}</div>}
            </div>
            <div>
              ...
              <input value={p.originalPrice.replace(/^Rs\.\s*/,"")} inputMode="numeric" placeholder="optional"
                onChange={e => setP({ ...p, originalPrice:fmtPrice(e.target.value) })}
                style={{ width:"100%", padding:"11px 13px", border:"1.5px solid #e2e8f0", borderRadius:9, fontSize:14, outline:"none", fontFamily:"inherit", boxSizing:"border-box" }} />
              {p.originalPrice && <div style={{ fontSize:11, color:"#94a3b8", marginTop:4 }}>{p.originalPrice}</div>}
            </div>
          </div>
          ...
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={() => saveProduct(p)} disabled={!p.name||!p.price||loading}
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f0fdf4" }}>
      {/* Toast */}
      {toast && (
        <div style={{ position:"fixed", top:16, left:"50%", transform:"translateX(-50%)", background:"#0f172a", color:"#fff", padding:"10px 24px", borderRadius:100, fontSize:13, fontWeight:700, zIndex:9999, boxShadow:"0 4px 20px rgba(0,0,0,0.25)", whiteSpace:"nowrap" }}>
          {toast}
        </div>
      )}
      ...
      <div style={{ background:"linear-gradient(135deg,#052e16,#14532d)", position:"sticky", top:0, zIndex:100, boxShadow:"0 2px 12px rgba(0,0,0,0.15)" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 16px", maxWidth:860, margin:"0 auto" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:10, color:"rgba(255,255,255,0.45)" }}>Battery Master · Supabase</div>
            </div>
          </div>
        ...
        <div style={{ display:"flex", borderTop:"1px solid rgba(255,255,255,0.07)", maxWidth:860, margin:"0 auto" }}>
          {[{n:products.length,l:"Total"},{n:products.filter(x=>x.stock==="In Stock").length,l:"In Stock"},{n:products.filter(x=>x.stock==="Limited").length,l:"Limited"},{n:products.filter(x=>x.badge&&x.badge!=="none").length,l:"Badged"}].map(s => (
          ...
          <>
            <div style={{ display:"flex", gap:10, marginBottom:12, flexWrap:"wrap" }}>
              ...
              <button onClick={loadProducts} disabled={loading}
                style={{ padding:"10px 12px", background:"#fff", color:"#64748b", border:"1.5px solid #e2e8f0", borderRadius:10, fontSize:12, fontWeight:600, cursor:"pointer", whiteSpace:"nowrap" }}>
                {loading ? "⏳" : "🔄 Refresh"}
              </button>
            </div>
            ...
            {loading && products.length===0 ? (
              <div style={{ background:"#fff", borderRadius:14, border:"1.5px solid #d1fae5", padding:"48px 20px", textAlign:"center", color:"#64748b" }}>
                <div style={{ fontSize:32, marginBottom:10 }}>⏳</div>
                <div style={{ fontSize:14, fontWeight:600 }}>Loading from Supabase...</div>
              </div>
            ) : filtered.length===0 ? (
              <div style={{ background:"#fff", borderRadius:14, border:"1.5px solid #d1fae5", padding:"48px 20px", textAlign:"center", color:"#94a3b8" }}>
                ...
                {products.length===0 && <button onClick={() => { setEditing(blank()); setTab("add"); }} style={{ marginTop:14, padding:"10px 20px", background:"#16a34a", color:"#fff", border:"none", borderRadius:8, fontSize:13, fontWeight:700, cursor:"pointer" }}>+ Add First Product</button>}
              </div>
            ) : (
                ...
                {filtered.map(p => (
                  <div key={p.id} style={{ background:"#fff", border:"1.5px solid #d1fae5", borderRadius:14, padding:"13px 14px", display:"flex", alignItems:"center", gap:12 }}>
                    <div style={{ width:54, height:54, flexShrink:0, borderRadius:10, background:"#f0fdf4", border:"1px solid #d1fae5", overflow:"hidden", display:"flex", alignItems:"center", justifyContent:"center" }}>
                      {p.image ? <img src={p.image} alt={p.name} style={{ width:"100%", height:"100%", objectFit:"contain", padding:4 }} /> : <span style={{ fontSize:20 }}>🔋</span>}
                    </div>
