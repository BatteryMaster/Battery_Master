...
function load():CartItem[]{if(typeof window==="undefined")return[];try{const s=localStorage.getItem("zeko-cart");return s?JSON.parse(s):[]}catch{return[];}}
export function CartProvider({children}:{children:React.ReactNode}){
  const [cartItems,setCartItems]=useState<CartItem[]>(load);
  useEffect(()=>{localStorage.setItem("zeko-cart",JSON.stringify(cartItems));},[cartItems]);
